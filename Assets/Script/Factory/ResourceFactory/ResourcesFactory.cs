...
public class ResourcesFactory : IResourceFactory
{
    ...
    {
        var controllers = Resources.LoadAll<RuntimeAnimatorController>(CharacterAnimatorPath)?
            .Where(x => x.name.Equals(name)).ToArray();
        
        if (controllers == null || controllers.Length == 0)
        {
            Debug.LogError($"找不到名为 {name} 的角色动画控制器，路径: {CharacterAnimatorPath}");
            return null;
        }
